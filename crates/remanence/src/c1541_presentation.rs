...
use crate::drive_profile::{C1541, DriveProfile, GroupCodec};
use crate::encoded_bytestream::{
...
use crate::flux_capture::SessionBacking;
use crate::flux_medium::{FluxMedium, LocationKey, MediumFactKind, Pulse};
use crate::hardware_bitstream::{
...
fn zone_of<'a>(
    ...
    denominator: u64,
    policy: &ReadChannelPolicy,
) -> Result<Option<(usize, &'a crate::drive_profile::DensityZone)>> {
...
mod tests {
    ...
    fn a_declared_byte_sequence_survives_the_whole_journey_unchanged() {
        ...
        let read: Vec<u8> = inner
            ...
            .iter()
            .filter_map(crate::encoded_bytestream::ByteRecord::value)
            .collect();

...
use crate::filesystem::fat::{FatEntry, FatVolume, VolumeDeclaration};
use crate::flux::load::{self as flux_load, CollectionMember, FluxState};
use crate::image::adapters::{
    self, ActiveLayer, DeviceIdentity, ImageFormatDescriptor, OpenedImage,
};
use crate::io::cache::SessionCache;
use crate::io::device::{AccessIntent, AccessMode, Claim, Device};
use crate::io::journal;
use crate::io::source::{self, ClaimedSource, Evidence, ImageSource};
...
fn no_archive_space(verb: &str, named: String) -> Error {
    Error::unsupported(format!(
...
mod tests {
    ...
    fn run_crashing_commit(path: &std::path::Path, boundary: &str) {
        let status = Command::new(std::env::current_exe().expect("test executable"))
            .arg("--ignored")
            .arg("--exact")
            .arg("model::disk::tests::crash_commit_child")
            .arg("--nocapture")

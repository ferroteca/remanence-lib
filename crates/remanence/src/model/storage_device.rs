...
//! or not a disk was in it, and "insert the disk" cannot hang off the
//! disk.
//!
...
use crate::model::media::{MediaId, MediaLink, MediaPool, Medium};
...
impl StorageDevice {
    ...
    /// configuration** — the machine held the drive whether or not a
    /// disk was in it — so this is the only way one is made, and linking
    /// a medium is a separate act.
    ...
    pub fn insert(&mut self, media: MediaId) -> Result<()> {
        ...
        if recorded != slot {
            return Err(Error::unsupported(format!(
                ...
                crate::model::device_type::recorded_reading(recorded.device_type()),
                served_reading(slot),
            )));
        }
...
mod tests {
    use super::*;
    use crate::model::device_type::{FloppyDrive, HardDrive};

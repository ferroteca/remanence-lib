...
use crate::c1541_presentation::{C1541Bitstream, C1541Bytestream, materialize_bitstream};
use crate::c1541_sectors::C1541Sectors;
use crate::device::{AccessMode, Claim, read_exact_at};
use crate::device_type::{DeviceType, FloppyDrive};
use crate::drive_profile::DriveProfile;
use crate::error::{Error, ErrorCategory, Result};
use crate::evidence::DeclaredLoss;
use crate::flux_medium::FluxMedium;
use crate::kryoflux::{self, MemberSource};
use crate::media_profile::MediaProfile;
use crate::remanence_reconstruction::{ReconstructionPolicy, RecordingSelection, plan};
use crate::session::{
...
impl FluxState {
    ...
    pub(crate) fn load_kryoflux(
            ...
            .map(|member| -> Result<Box<dyn MemberSource>> {
                match member {
                    CollectionMember::Handle(file) => {
                        let name = crate::handle::recovered_name(&file)
                            .and_then(|path| {
                                path.file_name()
        ...
        let recognition = crate::drive_profile::recognition(&assembled.capture, Some(profile.id))?;
    ...
    pub(crate) fn load_p64(
        ...
        let named = crate::media::named(path.as_deref());
        let (medium, report) = crate::p64::decode(source, &named, cache_bytes)?;
        ...
        Ok(Self {
            ...
            profile: &crate::drive_profile::C1541,
            format_id: "p64",
...
mod tests {
    ...
    use crate::flux_medium::{Derivation, FluxMedium, LocationKey, MediumBuilder, Pulse, Strength};
    ...
    fn p64_artifact(tag: &str) -> PathBuf {
        fn medium_of(pulses: &[(u64, u32)]) -> FluxMedium {
            let frame =
                crate::p64::container_frame(Provenance::new("p64").note("declared for the test"))
            ...
            struct Bytes(Vec<u8>);
            impl crate::flux_capture::ByteSource for Bytes {
        ...
        crate::p64::write_new_artifact(&medium, &path).expect("the artifact writes");
        path
    }

...
use crate::filesystem::catalog as filesystem_catalog;
use crate::filesystem::fat::FatVolume;
use crate::image::qcow2::{QCOW2_MAGIC, Qcow2, SUPPORTED_VERSION_CEILING};
use crate::image::vdi::{
...
impl ImageFormatAdapter for Qcow2Adapter {
    ...
    fn open_disk(&self, file: MediumDevice, path: Option<&Path>) -> Result<Box<dyn OpenedImage>> {
        Ok(Box::new(Qcow2Image(crate::image::qcow2::open_chain(
            file, path,
        )?)))
    }
}
...
fn volumes_of(
    ...
    let spans: Vec<(Option<u32>, u64, u64)> = match mbr::discover(device)? {
        ...
        mbr::Discovery::BareVolume => {
            let volume = crate::model::volume::direct(crate::model::volume::AddressedRegion {
...
impl ImageFormatAdapter for VdiAdapter {
    ...
    fn probe(&self, input: &ProbeInput<'_>) -> ProbeResult {
        ...
        // exist.
        let (major, minor) = crate::image::vdi::version(input.prefix);
    ...
    fn identify_filesystems(
        ...
        let header = crate::image::vdi::VdiHeader::parse(&mut device)?;
        evidence.push(format!(
        ...
        if header.image_type == crate::image::vdi::VdiImageType::Differencing {
    ...
    fn open_disk(&self, file: MediumDevice, path: Option<&Path>) -> Result<Box<dyn OpenedImage>> {
        Ok(Box::new(VdiImage(crate::image::vdi::open_chain(
            file, path,
        )?)))
    }
}

...
//! [`crate::archive::zip::ZipCatalog`] owns ZIP,
//! [`crate::archive::sevenzip::SevenZipCatalog`] owns 7z — and is the P12 adapter
...
//!
//! [`zip`] and [`sevenzip`] are the enrolled grammars, each reading in
//! place by positioned reads through the codecs the core owns.
...
use crate::archive::sevenzip::SEVENZIP_ADAPTER;
use crate::archive::zip::ZIP_ADAPTER;
use crate::error::{Error, ErrorCategory, Result};
use crate::filesystem::{Catalog, Entry, EntryFact, EntryKind};
use crate::io::device::{AccessIntent, AccessMode, Claim, open_locked, read_exact_at};
use crate::io::handle;
use crate::io::source::{self, ArchiveLayer, ClaimedSource, ImageSource};
use crate::model::assurance::{Assurance, AssuranceOutcome, ByteRange};
use crate::model::media_profile::{MediaProfile, VIRTUAL};
use crate::model::session::{Identification, Layer, LayerKind, LayerLayout, SizeInformation};
...
impl ArchiveRecognition {
    ...
    pub(crate) fn load(file: File, grammar: &str) -> Result<Self> {
        ...
        let named = crate::model::media::named(name.as_deref());

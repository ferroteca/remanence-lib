...
#[test]
fn the_lineage_artifact_opens_at_scale() {
    // The golden fixture is an artifact the research implementation
    // wrote from the same capture the repository's other flux tests key
    // off. It cannot be fetched, so its absence skips rather than
    // fails; prep_fixtures.py names where it comes from.
    let path = common::fixtures_dir().join("pinball-construction-set-c64.pooled.remanence");
    if !path.exists() {
        eprintln!("skipping: golden remanence fixture not present at {path:?}");
        return;
    }

    // A tight bound, so the working set is visibly narrower than the
    // image (P27) rather than incidentally so.
    let image = RemanenceImage::open_with_cache(&path, 1 << 20)
        .expect("the lineage artifact opens");
    let report = image.inspect();

    assert_eq!(report.form_factor, "5.25-inch");
    assert!(
        report.orbits.len() > 30,
        "a whole side reconstructs tens of orbits: {}",
        report.orbits.len()
    );
    let points: u64 = report.orbits.iter().map(|orbit| orbit.points).sum();
    assert!(
        points > 1_000_000,
        "a whole side carries over a million transitions: {points}"
    );
    assert!(
        report.orbits.iter().all(|orbit| orbit.radius_microns > 0),
        "every orbit sits at a positive radius"
    );

    // The points are never held whole: the declared bound is what the
    // working set answers to, not the size of the image.
    assert!(image.backing_bytes() > image.resident_bytes());

    // Re-encoded by this library and reopened, the image is unchanged.
    // Cross-implementation byte identity is not claimed; model identity
    // is.
    let destination = scratch("lineage-rewritten");
    let _ = std::fs::remove_file(&destination);
    let written = image.write(&destination).expect("the image re-encodes");
    assert_eq!(written.orbits, report.orbits.len() as u64);
    assert_eq!(written.points, points);

    let again = RemanenceImage::open(&destination).expect("our own artifact opens");
    assert_eq!(again.inspect().orbits, report.orbits);
    assert_eq!(again.inspect().holes, report.holes);

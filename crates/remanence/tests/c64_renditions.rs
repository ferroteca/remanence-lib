...
/// The renditions over the artifact the research lineage wrote from a
/// real capture, against the d64 and g64 it rendered from the same
/// disk. Checksummed sectors are the strongest comparison there is: any
/// block both renditions hold must be byte-identical, whatever
/// floating-point daylight separates the two reconstructions' angles.
/// None of it can be fetched, so absence skips rather than fails.
#[test]
fn the_lineage_disk_renders_to_the_lineage_artifacts() {
    let fixtures = common::fixtures_dir();
    let source = fixtures.join("pinball-construction-set-c64.pooled.remanence");
    if !source.exists() {
        eprintln!("skipping: golden remanence fixture not present at {source:?}");
        return;
    }
    let image = RemanenceImage::open(&source).expect("the lineage artifact opens");

    if let Ok(golden) = std::fs::read(fixtures.join("pinball-construction-set-c64.d64")) {
        let report = image.describe_d64().expect("the d64 is computed");
        assert!(
            report.blocks_read >= 600,
            "the reference disk reads most of its 683 blocks: {}",
            report.blocks_read
        );
        let (blocks, destination) = (683usize, scratch("lineage-d64", "d64"));
        let _ = std::fs::remove_file(&destination);
        image.write_d64(&destination).expect("the d64 writes");
        let mine = std::fs::read(&destination).expect("the artifact is there");

        let golden_present: Vec<bool> = if golden.len() > blocks * 256 {
            golden[blocks * 256..].iter().map(|&code| code == 1).collect()
        } else {
            vec![true; blocks]
        };
        let mine_present: Vec<bool> = if mine.len() > blocks * 256 {
            mine[blocks * 256..].iter().map(|&code| code == 1).collect()
        } else {
            vec![true; blocks]
        };
        let mut shared = 0usize;
        for index in 0..blocks {
            if golden_present[index] && mine_present[index] {
                let at = index * 256;
                assert_eq!(
                    &mine[at..at + 256],
                    &golden[at..at + 256],
                    "block {index} differs from the lineage"
                );
                shared += 1;
            }
        }
        ...
        eprintln!("skipping the d64 comparison: golden not present");
    }

    if let Ok(golden) = std::fs::read(fixtures.join("pinball-construction-set-c64.front.g64")) {
        let report = image.describe_g64().expect("the g64 is computed");
        assert_eq!(&golden[..8], b"GCR-1541");
        for half_track in &report.half_tracks {
            let offset_at = 12 + half_track.index as usize * 4;
            assert!(
                u32::from_le_bytes(golden[offset_at..offset_at + 4].try_into().unwrap()) != 0,
                "half-track {} present here but absent in the lineage",
                half_track.index
            );
            let zone_at = 12 + 84 * 4 + half_track.index as usize * 4;
            assert_eq!(
                u32::from(half_track.speed_zone),
                u32::from_le_bytes(golden[zone_at..zone_at + 4].try_into().unwrap()),
                "half-track {} zones differently from the lineage",
                half_track.index
            );
        }
    } else {
        eprintln!("skipping the g64 comparison: golden not present");
    }
}

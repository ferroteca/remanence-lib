...
const SKIP: &str = "REMANENCE_SKIP_CC";
...
impl Compiler {
    ...
    fn answers(program: &Path, bin_dir: Option<PathBuf>) -> Option<Self> {
...
fn find(override_var: &str, names: &[&str]) -> Option<Compiler> {
...
fn require(override_var: &str, names: &[&str], language: &str) -> Option<Compiler> {
...
fn compile(compiler: &Compiler, source: &Path, object: &str, extra: &[&str]) -> Result<(), String> {
    let output = compiler
        .command()

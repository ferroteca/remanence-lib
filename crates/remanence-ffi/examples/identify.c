...
static const char *outcome_name(RemanenceLetterOutcome outcome) {
    switch (outcome) {
        ...
        case REMANENCE_LETTER_OUTCOME_OPTICAL_DRIVE: return "optical-drive";
...
/* Reads the machine this medium sits in: which device it booted, which
 * operating system was installed there, and the drive letters that system
 * gave.
 *
 * Nothing here is asserted. The machine is the one we built by adding a
 * device and loading a medium into it; everything else -- which DOS, what
 * its CONFIG.SYS declared, which volume booted -- the library reads off
 * the disk. Passing NULL as the identity names the session's anonymous
 * machine, which is a machine like any other. */
static void show_drive_letters(RemanenceSession *session) {
    RemanenceErrorCategory error_category;
    char *error = NULL;
    char *error_rule = NULL;

    RemanenceMachine *machine = remanence_session_machine(session, NULL);
    if (machine == NULL) {
        printf("\nno machine to read\n");
        return;
    }
    ...
    RemanenceMachineReport *report =
        remanence_machine_inspect(machine, &error_category, &error, &error_rule);
    if (report == NULL) {
        report_error("\nerror reading the machine", error_category, error, error_rule);
        return;
    }

    printf("\nMachine: %s\n", remanence_machine_report_identity(report) != NULL
                                  ? remanence_machine_report_identity(report)
                                  : "(anonymous)");

    /* Which volume booted, and what settled it. */
    switch (remanence_machine_report_boot(report)) {
        case REMANENCE_BOOT_OUTCOME_BOOTED: {
            uint8_t major = 0;
            uint8_t minor = 0;
            printf("Booted:  %s on %s", remanence_machine_report_boot_system(report),
                   remanence_machine_report_boot_attachment(report));
            if (remanence_machine_report_boot_version(report, &major, &minor)) {
                printf(" version %" PRIu8 ".%02" PRIu8, major, minor);
            } else {
                printf(" version %s", remanence_machine_report_boot_version_state(report));
            }
            printf("%s\n", remanence_machine_report_boot_declared(report)
                               ? " (declared, not read)"
                               : "");
            break;
        }
        case REMANENCE_BOOT_OUTCOME_AMBIGUOUS:
            printf("Booted:  ambiguous -- several systems, nothing settling which ran\n");
            break;
        case REMANENCE_BOOT_OUTCOME_NOTHING_BOOTABLE:
            printf("Booted:  nothing this release recognizes\n");
            break;
    }

    /* The devices the machine holds, in the order it attached them. */
    size_t disks = remanence_machine_report_disk_count(report);
    for (size_t i = 0; i < disks; ++i) {
        const char *note = remanence_machine_report_disk_note(report, i);
        printf("  %s (%s)%s%s\n", remanence_machine_report_disk_attachment(report, i),
               remanence_machine_report_disk_family(report, i) != NULL
                   ? remanence_machine_report_disk_family(report, i)
                   : "no device type",
               note != NULL ? " -- " : "", note != NULL ? note : "");
    }

    size_t letters = remanence_machine_report_drive_count(report);
    printf("\nDOS drive letters (%zu):\n", letters);
    for (size_t i = 0; i < letters; ++i) {
        uint64_t volume = 0;
        RemanenceLetterOutcome outcome = remanence_machine_report_drive_outcome(report, i);
        printf("  %c: %s", remanence_machine_report_drive_letter(report, i),
               outcome_name(outcome));
        const char *attachment = remanence_machine_report_drive_attachment(report, i);
        if (attachment != NULL) {
            printf(" (%s)", attachment);
        }
        if (remanence_machine_report_drive_volume(report, i, &volume)) {
            printf(" volume %" PRIu64, volume);
        }
        if (outcome == REMANENCE_LETTER_OUTCOME_PHANTOM) {
            printf(" of %c:", remanence_machine_report_drive_phantom_of(report, i));
        }
        const char *reason = remanence_machine_report_drive_reason(report, i);
        if (reason != NULL) {
            printf(" -- %s", reason);
        }
        printf("\n");
    ...
    /* Every volume the machine holds, lettered or not: one with no letter
     * is still a volume, and the identity is what a file verb takes. */
    size_t volumes = remanence_machine_report_volume_count(report);
    printf("Volumes (%zu):\n", volumes);
    for (size_t i = 0; i < volumes; ++i) {
        char letter = remanence_machine_report_volume_letter(report, i);
        printf("  %" PRIu64 " on %s", remanence_machine_report_volume_id(report, i),
               remanence_machine_report_volume_attachment(report, i));
        if (letter != '\0') {
            printf(" as %c:", letter);
        } else {
            printf(" (no letter)");
    ...
    for (size_t i = 0; i < remanence_machine_report_provenance_count(report); ++i) {
        printf("  * %s\n", remanence_machine_report_provenance(report, i));
    }

    /* The machine view is borrowed -- the session owns it, so only the
     * report is ours to free. */
 ...
 * families, so a flux artifact is reached through its own type -- there
 * is no device to load it into. What crosses the boundary is the image's
 * shape; the points beneath it stay in the library, streamed from
 * private session storage under the declared bound. */
...
static int show_remanence_image(const char *path, const char *destination) {
    ...
    if (p64 == NULL) {
        ...
        uint64_t pulses = 0;
        size_t slots = remanence_p64_half_track_count(p64);
        for (size_t i = 0; i < slots; ++i) {
            RemanenceP64HalfTrack slot;
            if (remanence_p64_half_track(p64, i, &slot)) {
                pulses += slot.pulses;
            }
        }
...
static int write_renditions(const char *path, const char *stem) {
    ...
    if (p64 == NULL) {
        ...
        printf("Wrote:   %s (%zu half-tracks)\n", destination,
               remanence_p64_half_track_count(p64));
        remanence_p64_report_free(p64);
    }

...
disk.partition(0)                        // the direct partition
...
let mut fs = disk.partition(0)           // FAT12 by evidence — the boot
...
loaded image would, the drive catalog naming both PC families — the
5.25-inch 1.2 MB drive and the 3.5-inch 1.44 MB drive; and
`partition(0).filesystem()` opens FAT12 over it by evidence — the boot
record I recorded testifying for itself — through the very same door U31
writes a file through on a loaded disk. The file verbs are not new; the
arc reaches a door already built.
...
test that the arc recorded what it claimed. It answers `filesystem_as
("fat")` rather than `filesystem()`: raw bytes declare no partition
type, so nothing determines the namespace the way the medium's own
`record_as` did, and this journey's one caller who knows says so.

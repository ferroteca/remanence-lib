...
F34 is an ordinary P12 image-format adapter for P64, claimed in both directions. P64's authoritative image layer is a flux medium (P22), so the adapter owns the container's identity, recognition and evidence, version and structure validation, refusals, decoding into `FluxMedium`, and encoding of a mastered `FluxMedium` into a new artifact. It owns no selection, reconciliation, or timing policy: those are F33's, and the adapter refuses rather than repairs what arrives outside its claim.
...
Round trip, and it is a same-layer comparison because both ends are a `FluxMedium`: a fixture mastered by F33 encodes, reopens through this adapter's own decode, and presents the same half-tracks, pulse positions, and strengths. Encoding is deterministic, so the same mastered medium produces the same bytes.

...
- **Half-track map and admission.** How the set's source drive-step positions map onto 1541 half-tracks, and which of them are admitted as recorded. The fixture supplies 84 positions per side; that the mapping is two steps per 1541 track is a declared fact of the capture, not arithmetic the profile performs unasked. Admission is decided per location from the evidence the drive-profile probe reports, never from step parity (D12), and a source position no map covers is refused.
